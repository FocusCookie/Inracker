...
import { useEditNPC } from "@/hooks/useEditNPC";
import { useEffects } from "@/hooks/useEffects";

type OverlayProps = OverlayMap["encounter-npc.edit"];
...
function EditEncounterNPCDrawer({
  npc,
  open,
  onOpenChange,
  onExitComplete,
  ...
  const { t } = useTranslation("ComponentEncounterNPCEditDrawer");
  const openOverlay = useOverlayStore((s) => s.open);
  const [isEditing, setIsEditing] = useState(false);
  const [closingReason, setClosingReason] = useState<
    null | "success" | CancelReason
  >(null);
  const form = useEditNPC(npc);
  ...
  async function handleDelete() {
    try {
      ...
      onComplete({ ...npc, id: -1 } as EncounterNPC); 
      setClosingReason("success");
      onOpenChange(false);
    } catch (error) {
      console.error("Failed to delete encounter npc", error);
    } finally {
      setIsEditing(false);
    }
  }
  ...
  return (
    ...
    >
      <CreateNPCForm form={form} disabled={isEditing}>
...
export default EditEncounterNPCDrawer;
